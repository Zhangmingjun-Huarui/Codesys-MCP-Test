...
try:
    ...
    log("STEP7: Starting data collection (%d samples, %dms interval)..." % (sample_count, sample_interval))
    for i in range(sample_count):
        ...
        log("  Sample %d/%d: rSineOutput=%s, xSineRunning=%s" % (i + 1, sample_count, output_val, running))

    result = {"project": project_path, "collectedAt": time.time() * 1000, "samples": samples}
    with open(output_file, "w") as f:
        json.dump(result, f, indent=2)
    log("STEP8: Data saved to %s" % output_file)
    ...
    try:
        ...
        log("STEP9: History updated (%d records)" % len(history))
